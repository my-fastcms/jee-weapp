package com.dbumama.market.model.base;

import com.dbumama.market.model.gen.WxmModel;
import com.jfinal.plugin.activerecord.IBean;

/**
 * Generated by Wxmall, do not modify this file.
 * http://www.dbumama.com
 */
@SuppressWarnings({"serial", "unchecked"})
public abstract class BaseMenuReplyNews<M extends BaseMenuReplyNews<M>> extends WxmModel<M> implements IBean {

	public M setId(Long id) {
		set("id", id);
		return (M)this;
	}
	
	public Long getId() {
		return getLong("id");
	}

	public M setReplyConfigId(Long replyConfigId) {
		set("reply_config_id", replyConfigId);
		return (M)this;
	}
	
	public Long getReplyConfigId() {
		return getLong("reply_config_id");
	}

	public M setMsgOrgType(Integer msgOrgType) {
		set("msg_org_type", msgOrgType);
		return (M)this;
	}
	
	public Integer getMsgOrgType() {
		return getInt("msg_org_type");
	}

	public M setMediaId(String mediaId) {
		set("media_id", mediaId);
		return (M)this;
	}
	
	public String getMediaId() {
		return getStr("media_id");
	}

	public M setMsgTitle(String msgTitle) {
		set("msg_title", msgTitle);
		return (M)this;
	}
	
	public String getMsgTitle() {
		return getStr("msg_title");
	}

	public M setMsgDesc(String msgDesc) {
		set("msg_desc", msgDesc);
		return (M)this;
	}
	
	public String getMsgDesc() {
		return getStr("msg_desc");
	}

	public M setMsgPic(String msgPic) {
		set("msg_pic", msgPic);
		return (M)this;
	}
	
	public String getMsgPic() {
		return getStr("msg_pic");
	}

	public M setMsgUrl(String msgUrl) {
		set("msg_url", msgUrl);
		return (M)this;
	}
	
	public String getMsgUrl() {
		return getStr("msg_url");
	}

	public M setActive(Boolean active) {
		set("active", active);
		return (M)this;
	}
	
	public Boolean getActive() {
		return get("active");
	}

	public M setCreated(java.util.Date created) {
		set("created", created);
		return (M)this;
	}
	
	public java.util.Date getCreated() {
		return get("created");
	}

	public M setUpdated(java.util.Date updated) {
		set("updated", updated);
		return (M)this;
	}
	
	public java.util.Date getUpdated() {
		return get("updated");
	}

}
