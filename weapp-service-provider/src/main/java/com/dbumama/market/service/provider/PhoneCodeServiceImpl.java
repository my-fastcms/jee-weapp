...
import com.dbumama.market.notify.sms.SendSmsMessageUtil;
...
public class PhoneCodeServiceImpl implements PhoneCodeService{
	...
	public String getCodeByForget(String phone,String ip) throws UserException{
		...
		SellerUser user = sellerUserdao.findFirst("select * from "+SellerUser.table +" where phone =? ", phone.trim());
		if(user == null)
			throw new UserException("手机号码不存在");
		
		return sendCode(phone.trim(), ip);
     }
	...
	public String getCode(String phone,String ip) throws UserException {
		if(StrKit.isBlank(phone)) throw new UserException("手机号码不能为空");
		SellerUser user = sellerUserdao.findFirst("select * from " + SellerUser.table + " where phone=? ", phone.trim());
		if(user != null) throw new UserException("手机号码已注册");
		return sendCode(phone.trim(), ip);
	}
	...
	private String sendCode(String phone, String ip) throws UserException {
		...
		if(!bl) throw new UserException("验证码发送失败");
		
		//发送短信成功
		try{
			//插入
		    UserCode userCode = usercodedao.findFirst("select * from " + UserCode.table + " where vcode_phone=? ", phone);
		    if(userCode == null){
	...
	public String getCodeByCompanyPay(String phone, String ip) {
		if(StrKit.isBlank(phone)) throw new UserException("登录用户的手机号码为空");
		return sendCode(phone.trim(), ip);
	}
