...
import com.dbumama.market.service.api.ReceiverException;
...
@Bean
@RPCBean
public class BuyerReceiverServiceImpl extends WxmServiceBase<BuyerReceiver> implements BuyerReceiverService {
	/** 收货地址最大保存数 */
	...
	public List<BuyerReceiver> getBuyerReceiver(Long buyerId) {
		List<BuyerReceiver> receivers = receiverDao.find("select * from " + 
				BuyerReceiver.table + " where buyer_id=? order by id desc", buyerId);
	...
	public BuyerReceiver save(BuyerReceiverSubmitParamDto submitParam) throws ReceiverException {
		if(submitParam == null|| submitParam.getBuyerId() == null) throw new ReceiverException("还没有登陆，请重新授权登陆");
    	...
    	if(StrKit.isBlank(submitParam.getProvince()) || StrKit.isBlank(submitParam.getCity()) 
    			|| "请选择省".equals(submitParam.getProvince()) || "请选择市".equals(submitParam.getCity())){
