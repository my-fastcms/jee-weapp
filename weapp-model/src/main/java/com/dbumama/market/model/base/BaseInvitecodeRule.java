package com.dbumama.market.model.base;

import com.dbumama.market.model.gen.WxmModel;
import com.jfinal.plugin.activerecord.IBean;

/**
 * Generated by Wxmall, do not modify this file.
 * http://www.dbumama.com
 */
@SuppressWarnings({"serial", "unchecked"})
public abstract class BaseInvitecodeRule<M extends BaseInvitecodeRule<M>> extends WxmModel<M> implements IBean {

	public M setId(java.lang.Long id) {
		set("id", id);
		return (M)this;
	}
	
	public java.lang.Long getId() {
		return getLong("id");
	}

	public M setMissionRate(java.math.BigDecimal missionRate) {
		set("mission_rate", missionRate);
		return (M)this;
	}
	
	public java.math.BigDecimal getMissionRate() {
		return get("mission_rate");
	}

	public M setDiscountRate(java.math.BigDecimal discountRate) {
		set("discount_rate", discountRate);
		return (M)this;
	}
	
	public java.math.BigDecimal getDiscountRate() {
		return get("discount_rate");
	}

	public M setActive(java.lang.Boolean active) {
		set("active", active);
		return (M)this;
	}
	
	public java.lang.Boolean getActive() {
		return get("active");
	}

	public M setCreated(java.util.Date created) {
		set("created", created);
		return (M)this;
	}
	
	public java.util.Date getCreated() {
		return get("created");
	}

	public M setUpdated(java.util.Date updated) {
		set("updated", updated);
		return (M)this;
	}
	
	public java.util.Date getUpdated() {
		return get("updated");
	}

}
