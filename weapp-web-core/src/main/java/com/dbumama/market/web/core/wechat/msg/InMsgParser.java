...
package com.dbumama.market.web.core.wechat.msg;
...
//            }
        ...
        //小程序事件开始
  		if("weapp_audit_success".equals(event)){
  			InWeappAuditSuccessEvent e = new InWeappAuditSuccessEvent(toUserName, fromUserName, createTime, eventKey);
  			e.parse(xmlHelper);
  			return e;
  		}
  		if("weapp_audit_fail".equals(event)){
  			InWeappAuditFailEvent e = new InWeappAuditFailEvent(toUserName, fromUserName, createTime, eventKey);
  			e.parse(xmlHelper);
  			return e;
  		}
  		if("view_miniprogram".equals(event)){
  			InViewMiniprogramEvent e = new InViewMiniprogramEvent(toUserName, fromUserName, createTime, eventKey);
  			e.setMenuId(xmlHelper.getString("//MenuId"));
            e.setEventKey(xmlHelper.getString("//EventKey"));
  			return e;
  		}
  		if("user_enter_tempsession".equals(event)){
  			InUserEnterTempsessionEvent e = new InUserEnterTempsessionEvent(toUserName, fromUserName, createTime, eventKey);
  			e.setSessionFrom(xmlHelper.getString("//SessionFrom"));
  			return e;
  		}
  		//小程序事件结束
