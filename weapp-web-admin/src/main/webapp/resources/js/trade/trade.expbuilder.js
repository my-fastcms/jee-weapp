...
var ExpBuilder = {
	...
	initCanvas : function(){
		...
		+ '<img id="myexpbgimg" src="'+Trade.useExptpl.expBgimg+'"></div>';
		
		$("#form-container-div").append(bgHtml).append(Trade.useExptpl.expTplcontent);
	...
	saveDesignHtml : function(){
		...
		var expkey = Trade.useExptpl.expKey || $("input[name='myuseexpress']:checked").val();
		var expname = Trade.useExptpl.expName || $("input[name='myuseexpress']:checked").attr("expName");//exp_name
		
		var bgimg = Trade.useExptpl.expBgimg;
	...
	setExpbgimg : function(){
		...
		Trade.useExptpl.expBgimg = mynewexpimg;
