...
var Trade = {
	...
	getUserExpTemplate : function(){
		...
		if(!expbgimg || expbgimg==null || ""==expbgimg){
		...
		obz.ajaxJson(obz.ctx+"/express/getUserTplByKey", params, function(resp){
			...
			Trade.useExptpl.expBgimg = expbgimg; 	//注意：此处用用户临时选择的快递图片进行替换
	...
	displayTplImagesByCompkey : function(){
		...
		obz.ajaxJson(obz.ctx + "/express/getTplBgImagesByCompkey", params, function(resp){
			...
			for(var i=0;i<result.length;i++){
				var s = $('<span style="padding-left: 10px;padding-top: 5px;cursor: pointer;" id="span_'+i+'"></span>').append($('<img width="200px" height="120px" src="'+result[i].expImg+'">'));
		...
		try{
			...
			if(printData && printData != null){
				for(var i=0; i<printData.length;i++){
					var newDesignhtml = obz.dataTemplate4obj(Trade.useExptpl.expDesignhtml, printData[i]);
					...
					LODOP.ADD_PRINT_SETUP_BKIMG("<img border='0' " + "src='"+Trade.useExptpl.expBgimg+"' />");
					LODOP.ADD_PRINT_HTM(0,0,Trade.useExptpl.pagewidth+"mm",Trade.useExptpl.pageheight+"mm",printHtml);
				}
			}else{
				LODOP.ADD_PRINT_SETUP_BKIMG("<img border='0' " + "src='"+Trade.useExptpl.expBgimg+"' />");
				LODOP.ADD_PRINT_HTM(0,0,Trade.useExptpl.pagewidth+"mm",Trade.useExptpl.pageheight+"mm", Trade.useExptpl.expTplcontent);
			}
			LODOP.PREVIEW();	
		}catch(err){
			obz.showMessage(err);
		...
		try{
			...
			if(printData && printData != null){
				for(var i=0;i<printData.length;i++){
					...
					//打印常规文本数据
					var newDesignhtml = obz.dataTemplate4obj(Trade.useInvtpl.invDesignhtml, currTrade);
					//获取数据填充到模板中
					...
					var imgDesignHtml = $(Trade.useInvtpl.invImgDesignhtml);
					...
					var tableView = $(Trade.useInvtpl.invTabelhtml);
