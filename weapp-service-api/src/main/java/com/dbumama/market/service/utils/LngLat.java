...
/**
 * @author wangjun 2019年1月4日
 */
@SuppressWarnings("serial")
public final class LngLat implements Cloneable, Serializable {
	/**
	 * 纬度 (垂直方向)
	 */
	...
	public LngLat(double longitude, double latitude, boolean isCheck) {
		if (isCheck) {
			if ((-180.0D <= longitude) && (longitude < 180.0D))
				...
				// this.longitude = parse(((longitude - 180.0D) % 360.0D +
				// 360.0D) %
				// 360.0D - 180.0D);
			}
			...
			// this.latitude = parse(Math.max(-90.0D, Math.min(90.0D,
			// latitude)));
		} else {
			this.latitude = latitude;
